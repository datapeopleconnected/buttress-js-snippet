...
class MicrosoftAuth {
  ...
  async microsoftOAuth2Callback(microsoftKey, domainsKey, allowedMembers = []) {
    ...
    if (!microsoftAuthLambda) {
      const error = new Error('Can not find the microsoft lambda');
      error.code = 404;
      throw error;
    }
    ...
    if (!state || (state && state.value !== lambda.req.query.state)) {
      const error = new Error('State request not found or state request and state response do not match');
      error.code = 404;
      throw error;
    }
    if (!codeVerifier) {
    ...
    if (!buttressUser) {
      ...
      if (buttressUser && buttressUser.auth.some((u) => u.email !== profile.mail)) {
        lambda.logError(`AUTH: User ${profile.forename} found in the database is not matching the profile email ${profile.mail}`);
        const error = new Error(`AUTH: User ${profile.forename} found in the database is not matching the profile email ${profile.mail}`);
        error.code = 404;
        throw error;
      }
    ...
    if (!buttressUser) {
      ...
      if (allowedMembers.length < 1) {
        lambda.logError(`AUTH: User ${profile.forename} can not be authorised ${profile.mail}, the app do not have an allowed list`);
        const error = new Error(`AUTH: User ${profile.forename} can not be authorised ${profile.mail}, the app do not have an allowed list`);
        error.code = 401;
        throw error;
      }
