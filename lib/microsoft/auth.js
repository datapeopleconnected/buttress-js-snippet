...
class MicrosoftAuth {
  ...
  async microsoftGetOAuth2URL(microsoftKey) {
    ...
    const issuer = microsoftKey.getValue('ISSUER');
    const clientId = microsoftKey.getValue('CLIENT_ID');
    const redirectURI = microsoftKey.getValue('REDIRECT_URI');
    const scope = microsoftKey.getValue('SCOPE');
  ...
  async microsoftOAuth2Callback(microsoftKey, domainsKey, allowedMembers = []) {
    ...
    const issuer = microsoftKey.getValue('ISSUER');
    const clientId = microsoftKey.getValue('CLIENT_ID');
    const clientSecret = microsoftKey.getValue('CLIENT_SECRET');
    const redirectURI = microsoftKey.getValue('REDIRECT_URI');
    const scope = microsoftKey.getValue('SCOPE');
    ...
    const buttressURL = domainsKey.getValue('buttressURL');
    ...
    if (!buttressUser) {
      ...
      const allowedMember = allowedList.find((t) => profile.mail === t.email);
