...
class GoogleMail {
  ...
  async send(serviceAccount, email) {
    const secureStore = serviceAccount.storeData.reduce((output, obj) => {
      const data = obj.data;
      Object.keys(data).forEach((key) => {
        if (key.toUpperCase() === 'ISS') {
          output[key.toUpperCase()] = data[key];
        }
        if (key.toUpperCase() === 'SUB') {
          output[key.toUpperCase()] = data[key];
        }
        if (key.toUpperCase() === 'PRIVATE_KEY') {
          output[key.toUpperCase()] = data[key];
        }
      });
    ...
    await this._attemptDispatch(secureStore, email);
  }
