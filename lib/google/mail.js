...
class GoogleMail {
  ...
  async _getAuthToken(serviceAccount) {
    ...
    const objectKeysSA = Object.keys(serviceAccount);

    const claimSet = {
      iss: serviceAccount[objectKeysSA.find(key => key.toUpperCase() === 'ISS')],
      iat: new Date().getTime() / 1000,
      exp: new Date().getTime() / 1000 + (60 * 60),
      scope: 'https://mail.google.com/',
      aud: 'https://oauth2.googleapis.com/token',
      sub: serviceAccount[objectKeysSA.find(key => key.toUpperCase() === 'SUB')],
    };
    ...
    const signature = await lambdaAPI('cryptoCreateSign', {
      ...
      key: serviceAccount[objectKeysSA.find(key => key.toUpperCase() === 'PRIVATE_KEY')],
      encodingType: 'base64',
    });
