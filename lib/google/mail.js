...
class GoogleMail {
   ...
   * @return {Promise}
   */
  async send(email, credentials, tokens) {
    if (email.provider.name !== 'GOOGLE') {
      lambda.logError(`Can not find send an email that does not have Google as its provider`);
      throw new Error(`Can not find send an email that does not have Google as its provider`);
    }

    email = await this._getEmailTemplate(email);
    await this._attemptDispatch(email, credentials ,tokens);
  }
   ...
   * @return {Promise}
   */
  async _attemptDispatch(email, credentials, tokens) {
  ...
  async _dispatchEmail(tokens, headers, params, labelIds = []) {
