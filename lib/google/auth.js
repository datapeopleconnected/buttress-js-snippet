...
class GoogleAuth {
  ...
  async googleOAuth2Callback(googleKey, domainsKey, allowedMembers = [], tokens = null, profile = null) {
    ...
    if (!buttressUser) {
      ...
      const updates = [];
      authIndex = buttressUser.auth.findIndex((a) => a.app === 'google' && (a.appId === profile.sub));
    ...
    return {
      user: buttressUser,
      profile: profile,
      hasRefreshToken: (authIndex !== -1 && buttressUser?.auth[authIndex]?.refreshToken) ? true : (tokens?.refreshToken) ? true : false,
    };
  }
