...
class GoogleAuth {
  ...
  async googleGetOAuth2URL(googleKey) {
    ...
    const clientId = googleKey.getValue('CLIENT_ID');
    const redirectURI = googleKey.getValue('REDIRECT_URI');
    const scope = googleKey.getValue('SCOPE');
  ...
  async googleOAuth2Callback(googleKey, domainsKey, allowedMembers = []) {
    ...
    const clientId = googleKey.getValue('CLIENT_ID');
    const clientSecret = googleKey.getValue('CLIENT_SECRET');
    const redirectURI = googleKey.getValue('REDIRECT_URI');
    ...
    if (!buttressUser) {
      ...
      const allowedMember = allowedList.find((t) => profile.email === t.identifierEmail && profile.email_verified);
