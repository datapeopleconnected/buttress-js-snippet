...
class GoogleAuth {
   ...
   * @return {Promise}
   */
  async googleGetOAuth2URL(googleKey, state = null) {
    if (!googleKey) {
    ...
    return `https://accounts.google.com/o/oauth2/v2/auth/identifier?approval_prompt=force&client_id=${clientId}&redirect_uri=${redirectURI}&include_granted_scopes=${includeGrantedScopes}&response_type=${responseType}&scope=${scope}&access_type=${accessType}&state=${lambdaInfo.executionId}`;
  }
  ...
  async googleOAuth2Callback(googleKey, domainsKey, allowedMembers = [], tokens = null, profile = null) {
    ...
    if (!buttressUser) {
      ...
      if (buttressUser && buttressUser.auth.every((u) => u.email !== profile.email)) {
        lambda.logError(`AUTH: User ${profile.forename} found in the database is not matching the profile email ${profile.email}`);
        const error = new Error(`AUTH: User ${profile.forename} found in the database is not matching the profile email ${profile.email}`);
        error.code = 404;
        throw error;
      }
