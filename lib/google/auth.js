...
class GoogleAuth {
   ...
   * @param {string} userAuthAppId
   * @return {Promise}
   */
  async googleGetOAuth2URL(googleKey, userAuthAppId = null) {
    if (!googleKey) {
    ...
    let url = `https://accounts.google.com/o/oauth2/v2/auth/identifier?approval_prompt=force&client_id=${clientId}&redirect_uri=${redirectURI}&include_granted_scopes=${includeGrantedScopes}&response_type=${responseType}&scope=${scope}&access_type=${accessType}`;
    url = (userAuthAppId) ? `${url}&state=${userAuthAppId}` : url;
    return url;
  }
  ...
  async googleOAuth2Callback(googleKey, domainsKey, allowedMembers = []) {
    ...
    const state = lambda.req.query.state;
    ...
    try {
      lambda.logDebug(`AUTH: Finding user google, ${profile.sub}`);
      buttressUser = await Buttress.User.findUser('google', profile.sub);
    } catch (err) {
      if (err?.statusCode) {
    ...
    if (!buttressUser) {
      ...
      const updates = [];
      let authIndex = buttressUser.auth.findIndex((a) => a.app === 'google' && (a.appId === profile.sub || (state && a.appId === state)));
      ...
      if (authIndex === -1) {
        ...
        updates.push({path: `auth.${authIndex}.username`, value: userAuth.username});
        updates.push({path: `auth.${authIndex}.token`, value: userAuth.token});
        updates.push({path: `auth.${authIndex}.profileUrl`, value: userAuth.profileUrl});
        updates.push({path: `auth.${authIndex}.images.profile`, value: userAuth.profileImgUrl});
        updates.push({path: `auth.${authIndex}.locale`, value: userAuth.locale});
      }

      if (userAuth.refreshToken) {
        updates.push({path: `auth.${authIndex}.refreshToken`, value: userAuth.refreshToken});
      }
