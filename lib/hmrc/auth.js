...
class HMRCAuth {
   ...
   * @return {Promise}
   */
  async HMRCOAuth2Callback(nodeEnv, HMRCKey, domainsKey, allowedMembers = [], user = null, fraudPreventionHeaders = {}) {
    try {
      if (!HMRCKey) {
        const error = new Error('Missing required secure store data');
        error.code = 400;
   ...
   * @return {Promise}
   */
  async refreshAccessToken(user, HMRCKey, nodeEnv, fraudPreventionHeaders = {}) {
    try {
