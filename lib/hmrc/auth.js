...
class HMRCAuth {
  ...
  constructor() {
    this.__OAUTH_CALLBACK_ERROR_MESSAGES = {
      // oAuth errors
      ACCESS_DENIED: 'You declined access to HMRC — please sign in again and approve the request so we can verify your company.',
      'UNSUPPORTED GRANT_TYPE': 'Sign-in failed — the authentication flow requested is not supported by HMRC. Please contact nodeStream admin.',
    };
  }
   ...
   * @param {string} piiMasterKey
   * @param {Object} user
   * @param {Object} fraudPreventionHeaders
   * @return {Promise}
   */
  async HMRCOAuth2Callback(nodeEnv, HMRCKey, domainsKey, piiMasterKey, user = null, fraudPreventionHeaders = {}) {
    try {
      if (!HMRCKey) {
        const error = new Error('Missing required secure store data');
        error.code = 400;
      ...
      const queryError = lambda.req.query.error;
      if (queryError) {
        const err = new Error(queryError);
        const errorCode = lambda.req.query?.error_code;
      ...
      const vrnMetadata = execution.metadata.find((m) => m.key === 'VRN');
      if (!vrnMetadata) throw new Error(`Can not find VRN in the lambda execution`);
      const vrn = vrnMetadata.value;
      ...
      if (!authResult?.body?.access_token || !authResult?.body?.refresh_token) {
        throw new Error('HMRC token exchange did not return usable access/refresh tokens');
      }
      ...
      const storedToken = await Helpers.encryptToken(tokens.accessToken, piiMasterKey);
      const storedRefreshToken = await Helpers.encryptToken(tokens.refreshToken, piiMasterKey);
      ...
      return { user: buttressUser, accessToken: tokens.accessToken };
    } catch (err) {
      ...
      err.message = (errMsg && this.__OAUTH_CALLBACK_ERROR_MESSAGES[errMsg.toUpperCase()]) ? this.__OAUTH_CALLBACK_ERROR_MESSAGES[errMsg.toUpperCase()] : errMsg;
      err.code = (err.error_code) ? err.error_code : err.code;
      err.isUserDeclined = errMsg === 'access_denied';
   ...
   * @return {Promise}
   */
  async refreshAccessToken(user, HMRCKey, nodeEnv, piiMasterKey, fraudPreventionHeaders = {}) {
    try {
      ...
      const storedRefreshToken = await Helpers.decryptToken(hmrcAuth.refreshToken, piiMasterKey);
      ...
      const encryptedAccessToken = await Helpers.encryptToken(tokens.access_token, piiMasterKey);
      const encryptedRefreshToken = await Helpers.encryptToken(tokens.refresh_token, piiMasterKey);
