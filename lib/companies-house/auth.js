...
class CompaniesHouseAuth {
  ...
  async companiesHouseOAuth2Callback(nodeEnv, companiesHouseKey, domainsKey, allowedMembers = [], user = null) {
    try {
      ...
      if (lambda.req.query.error) throw new Error(queryError);
      ...
      let humanReadableError = (errCode && this.__OAUTH_CALLBACK_ERROR_MESSAGES[errCode.toUpperCase()]) ? this.__OAUTH_CALLBACK_ERROR_MESSAGES[errCode.toUpperCase()] : null;
      humanReadableError = (humanReadableError) ? humanReadableError : (errMsg && this.__OAUTH_CALLBACK_ERROR_MESSAGES[errMsg.toUpperCase()]) ? this.__OAUTH_CALLBACK_ERROR_MESSAGES[errMsg.toUpperCase()] : errMsg;
