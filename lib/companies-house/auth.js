...
class CompaniesHouseAuth {
   ...
   * @return {Promise}
   */
  async companiesHouseGetOAuth2URL(companiesHouseKey) {
    ...
    const scope = companiesHouseKey.getValue('SCOPE');
    if (!clientId || !redirectURI || !scope) {
      throw new Error('Missing some Comapnies House credentials');
    }

    return `https://identity.company-information.service.gov.uk/oauth2/authorise?client_id=${clientId}&redirect_uri=${redirectURI}&scope=${scope}&response_type=code`;
  }
