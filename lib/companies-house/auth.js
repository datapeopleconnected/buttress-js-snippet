...
class CompaniesHouseAuth {
   ...
   * @return {Promise}
   */
  async companiesHouseGetOAuth2URL(companiesHouseKey, scope = null, state = null) {
    ...
    const environment = await Buttress.getCollection('secureStore').findByName('environment');
    const nodeEnv = environment.getValue('nodeEnv');
   ...
   * @return {Promise}
   */
  async companiesHouseOAuth2Callback(companiesHouseKey, domainsKey, allowedMembers = [], user = null) {
    try {
      if (!companiesHouseKey) {
        const error = new Error('Missing required secure store data');
        error.code = 400;
      ...
      const environment = await Buttress.getCollection('secure-store').findByName('environment');
      const nodeEnv = environment.getValue('nodeEnv');
