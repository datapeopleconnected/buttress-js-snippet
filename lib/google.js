...
class Google {
   ...
   * @return {Promise}
   */
  async googleOAuth2Callback(googleKey, allowedMembers, code, domains) {
    ...
    const authResult = await lambdaAPI('fetch', {
      url: `https://oauth2.googleapis.com/token?code=${code}&client_id=${clientId}&client_secret=${clientSecret}&redirect_uri=${redirectURI}&grantType=${grantType}`,
      options: {
        method: 'POST',
        headers: {
